...
function answerClicked() {
  var index = getRandInteger(0, 65);
  var range = getRandInteger(1, 3);
  var multiplier = getMultiplier();
  var beforeafter = getBeforeAfter(multiplier);
  document.getElementById('question_content').innerHTML =
    'Which book comes <div class="emphasis">' +
    range +
    '</div> book(s) <br><div class="emphasis">' +
    beforeafter +
    '</div> <div class="emphasis"><u>' +
    books[index] +
    '</u></div>?';
  ...
  if (answerIndex >= 0 && answerIndex <= 65) {
    document.getElementById('answer_content').innerHTML =
      'The answer is<br><div class="emphasis">' +
      books[answerIndex] +
      '</div>!';
  } else {
